...
{
	return new QLabel( HeTypeDefs::prettyName( t ) + QChar(':'), p );
}

PersonPropsDlg::PersonPropsDlg( QWidget* p ):QDialog(p)
{
	setWindowTitle( tr("Edit Person - Herald") );
	...
	// Business Pane
	{
		...
		grid->addWidget( _label( this, AttrPostalAddr ), row, 0 );
	...
	// Private Pane
	{
		...
		grid->addWidget( _label( this, AttrPostalAddrPriv ), row, 0 );
...
void PersonPropsDlg::loadFrom( Udb::Obj& o )
{
...
void PersonPropsDlg::saveTo( Udb::Obj& o ) const
{
	...
	o.setString( AttrPostalAddr, d_addr->toPlainText().trimmed() );
	o.setString( AttrPhonePriv, d_phonePriv->text().simplified() );
	o.setString( AttrMobilePriv, d_mobilePriv->text().simplified() );
	o.setString( AttrPostalAddrPriv, d_addrPriv->toPlainText().trimmed() );
...
bool PersonPropsDlg::edit( Udb::Obj& item )
{
	setWindowTitle( tr("Edit Person - Herald") );
	loadFrom( item );
	bool run = true;
...
void PersonPropsDlg::setText(const QString &text)
{
    d_text->setText( text );
}
